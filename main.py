                ...
                if sheet_stripped in xl2_sheets_stripped:
                    ...
                    if df1 is not None and df2 is not None and '品番' in df1.columns and '個数' in df2.columns:
                        ...
                        for row_num in range(2, ws.max_row + 1):
                            ctype = ws.cell(row=row_num, column=t_idx).value if t_idx else ""
                            if ctype == '삭제':
                                for col_num in range(1, ws.max_column + 1):
                                    ws.cell(row=row_num, column=col_num).fill = red_fill
                            elif ctype in ['신규 추가', '개수 변경'] and n_idx:
                                ws.cell(row=row_num, column=n_idx).fill = yellow_fill
                        
                        # 열 너비 자동 조절
                        for column_cells in ws.columns:
                            length = max(len(str(cell.value or "")) for cell in column_cells)
                            ws.column_dimensions[column_cells[0].column_letter].width = max(length + 3, 12)
                        
                        summary_results.append({
                            '시트명': sheet, '신규': len(new), '변경': len(changed), '삭제': len(deleted), '총': len(final)
                        })

            # 생성된 시트가 없을 경우 안내 시트 강제 생성 (IndexError 방지)
            if sheets_found_count == 0:
                pd.DataFrame({'결과': ['품번(品番)과 개수(個数) 항목이 적힌 행을 찾지 못했습니다.']}).to_excel(writer, sheet_name='분석 결과 없음', index=False)
